...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname 114-sigs) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(check-expect (si-control (make-aim (make-posn 100 20) (make-tank 100 TANK-SPEED))
                          "left")
              (make-aim (make-posn 100 20) (make-tank 100 (- TANK-SPEED))))
(check-expect (si-control (make-fired (make-posn 100 20) (make-tank 100 TANK-SPEED) (make-posn 88 150))
                          "left")
              (make-fired (make-posn 100 20) (make-tank 100 (- TANK-SPEED)) (make-posn 88 150)))
...
; fire the missile
(check-expect (si-control (make-aim (make-posn 70 20) (make-tank 88 TANK-SPEED))
                          " ")
              (make-fired (make-posn 70 20) (make-tank 88 TANK-SPEED) (make-posn 88 MISSILE-INIT-Y)))

; can't fire another missile
(check-expect (si-control (make-fired (make-posn 70 20) (make-tank 88 TANK-SPEED) (make-posn 90 150))
                          " ")
              (make-fired (make-posn 70 20) (make-tank 88 TANK-SPEED) (make-posn 90 150)))

; ignore other keys
(check-expect (si-control (make-aim (make-posn 100 20) (make-tank 70 TANK-SPEED))
                          "up")
              (make-aim (make-posn 100 20) (make-tank 70 TANK-SPEED)))

(define (si-control s ke)
  (cond
    [(aim? s)
     (cond
       [(key=? ke "left") (make-aim (aim-ufo s) (steer-tank-left (aim-tank s)))]
       [(key=? ke "right") (make-aim (aim-ufo s) (steer-tank-right (aim-tank s)))]
       [(key=? ke " ") (make-fired (aim-ufo s) (aim-tank s) (fire-missile (aim-tank s)))]
       [else s])]
    [(fired? s)
     (cond
       [(key=? ke "left") (make-fired (fired-ufo s) (steer-tank-left (fired-tank s)) (fired-missile s))]
       [(key=? ke "right") (make-fired (fired-ufo s) (steer-tank-right (fired-tank s)) (fired-missile s))]
       [else s])]))
