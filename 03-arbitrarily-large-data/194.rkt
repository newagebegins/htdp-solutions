...
(check-expect (connect-dots MT (first triangle-p) triangle-p)
              (scene+line
               (scene+line
...
(check-expect (connect-dots MT (first square-p) square-p)
              (scene+line
               (scene+line
                (scene+line
...
(define (connect-dots img x p)
  (cond
    [(empty? (rest p)) (render-line img x (first p))]
    [else (render-line (connect-dots img x (rest p))
                       (first p)
                       (second p))]))
