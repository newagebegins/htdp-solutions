...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname 223-simple-tetris) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
; Lanscape -> Boolean
; return #true if at least one of the blocks in the lanscape is at the top row
(check-expect (touches-ceiling? '()) #false)
(check-expect (touches-ceiling? (list (make-block 3 2))) #false)
(check-expect (touches-ceiling? (list (make-block 0 0))) #true)
(check-expect (touches-ceiling? (list (make-block 5 1) (make-block 6 0) (make-block 3 3))) #true)

(define (touches-ceiling? l)
  (cond
    [(empty? l) #false]
    [else (or (zero? (block-y (first l)))
              (touches-ceiling? (rest l)))]))

; Tetris -> Boolean
; return #true if one of the columns contains enough blocks to touch the top of the canvas
(check-expect (game-over? (make-tetris (make-block 0 0) '())) #false)
(check-expect (game-over? (make-tetris (make-block 2 3)
                                       (list (make-block 0 0))))
              #true)
(check-expect (game-over? (make-tetris (make-block 2 3)
                                       (list (make-block 5 1) (make-block 6 0) (make-block 3 3))))
              #true)

(define (game-over? t)
  (touches-ceiling? (tetris-landscape t)))
