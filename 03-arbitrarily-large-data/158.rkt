...
; ShotWorld -> ShotWorld
; moves each shot on w up by one pixel,
; eliminates shots outside the canvas
(check-expect (tock.v2 '()) '())
(check-expect (tock.v2 (cons 50 '())) (cons 49 '()))
(check-expect (tock.v2 (cons 77 (cons 50 '()))) (cons 76 (cons 49 '())))
(check-expect (tock.v2 (cons -1 '())) '())
(check-expect (tock.v2 (cons 77 (cons -2 '()))) (cons 76'()))

(define (tock.v2 w)
  (cond
    [(empty? w) '()]
    [(cons? w) (if (negative? (first w))
                   (tock.v2 (rest w))
