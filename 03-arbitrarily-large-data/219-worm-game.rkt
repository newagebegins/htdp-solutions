...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname 219-worm-game) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; N N -> List-of-posns
; create one row of cells, ri is the row index, col# is the number of columns
(check-expect (row 4 0) '())
(check-expect (row 4 3) (list (make-posn 0 4) (make-posn 1 4) (make-posn 2 4)))

(define (row ri col#)
  (cond
    [(zero? col#) '()]
    [else (append (row ri (sub1 col#))
                  (list (make-posn (sub1 col#) ri)))]))
...
(define (all-cells col# row#)
  (cond
    [(zero? row#) '()]
    [else (append (all-cells col# (sub1 row#))
                  (row (sub1 row#) col#))]))
...
(define (empty-cells all seg)
  (cond
    [(empty? seg) all]
    [else (remove (first seg) (empty-cells all (rest seg)))])) 
