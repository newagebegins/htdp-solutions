...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname 469-solve) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
; A Matrix is one of:
;  – (cons Row '())
;  – (cons Row Matrix)
...
(define (solve tm)
  (local ((define e1 (first tm)))
    (cond
      [(empty? (rest tm)) (list (/ (second e1) (first e1)))]
      [else (local ((define s (solve (rest tm))))
              (cons (/ (- (rhs e1) (plug-in (rest (lhs e1)) s)) (first e1))
                    s))])))
